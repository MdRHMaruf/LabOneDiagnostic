...
public class FTraialBalance extends JPanel{
	...
	String col[]={"                                                             PERTICULAR","                DEBIT","                  CREDIT"};
	Object row[][]={};
	...
	int k=0;
	public FTraialBalance(SessionBeam sessionbeam) {
				...
				if (evt.getButton() == java.awt.event.MouseEvent.BUTTON3) {
					...
					if(confrim==JOptionPane.YES_OPTION){
						if(table.getSelectedColumn()==0){
							if(table.getValueAt(table.getSelectedRow(), 0).toString().trim().equals("(+) Balance brought forward from last year accounts") || table.getValueAt(table.getSelectedRow(), 0).toString().trim().equals("Total")){
	...
	private void btnprintEvent(){
		try {
			...
			String input = "AccountReport/TrialBalanceFinal.jrxml";
	...
	private void CreditLedgerNewDetails(int row,int col){
		try {
			...
			for(int b=0;b<ledgerlist.size();b++){
				int flag=0;
				ResultSet flagrs=db.sta.executeQuery("select pheadId from accfledger where ledgerId='"+ledgerlist.get(b)+"'");
				while(flagrs.next()){
					if(flagrs.getString("pheadId").toString().equals("6") || flagrs.getString("pheadId").toString().equals("7") || flagrs.getString("pheadId").toString().equals("8") || flagrs.getString("pheadId").toString().equals("17")){
						flag=1;
						break;
					}
				}
				if( ledgerlist.get(b).toString().equals("561") || ledgerlist.get(b).toString().equals("565") || ledgerlist.get(b).toString().equals("468") || ledgerlist.get(b).toString().equals("476") || ledgerlist.get(b).toString().equals("2") || ledgerlist.get(b).toString().equals("3")  || flag==1){
...
/*						select headid,pheadId from accfhead where headTitle='Observation Patient'
						select headid,pheadId from accfhead where headTitle='Outdoor Patient'
						select headid,pheadId from accfhead where headTitle='Indoor Patient'
						select headid,pheadId from accfhead where headTitle='Corporate'*/
				...
				if(ledgerlist.get(b).toString().equals("2") || ledgerlist.get(b).toString().equals("3") || ledgerlist.get(b).toString().equals("468") || ledgerlist.get(b).toString().equals("476") || flag==1){
		...
		try {
			...
			for(int count=0;count<FirstHeadRevenue.size();count++){
					...
					if(find!=0){
						...
						String id="";
						ResultSet result=db.sta.executeQuery("select headid from accfhead where headTitle='"+FirstHeadRevenue.get(count)+"'");
						while(result.next()){
							id=result.getString("headid");
						}
						findSubhead(id);
						//System.out.println("list "+findlist);
						findLedger();
						CreditLedgerNewDetails(k-1,2);
						findlist.clear();
						sum1=sum1+Double.parseDouble(table.getValueAt(k-1, 2).toString());
					}
			...
			ArrayList FirstHeadLaibilies=new ArrayList();
			ArrayList FirstHeadLaibiliesId=new ArrayList();
			ResultSet rs1=db.sta.executeQuery("select accfhead.headid,accfhead.headTitle from accfhead where accfhead.pheadId=4");
			while(rs1.next()){
				FirstHeadLaibilies.add(rs1.getString("headTitle"));
				FirstHeadLaibiliesId.add(rs1.getString("headid"));
			}
			
			for(int count=0;count<FirstHeadLaibilies.size();count++){
				ResultSet rs5=db.sta.executeQuery("select accfhead.headid,accfhead.headTitle from accfhead where accfhead.pheadId='"+FirstHeadLaibiliesId.get(count)+"'");
				int find=0;
				table.setValueAt("   *"+FirstHeadLaibilies.get(count), k, 0);
				k++;
				nrow++;
				int j=k;
				while(rs5.next()){
				...
				if(find!=0){
					...
					String id="";
					ResultSet result=db.sta.executeQuery("select headid from accfhead where headTitle='"+FirstHeadLaibilies.get(count)+"'");
					while(result.next()){
						id=result.getString("headid");
					}
					findSubhead(id);
			...
			ArrayList FirstHeadAsset=new ArrayList();
			ArrayList FirstHeadAssetId=new ArrayList();
			ResultSet rs2=db.sta.executeQuery("select accfhead.headid,accfhead.headTitle from accfhead where accfhead.pheadId=1");
			while(rs2.next()){
				FirstHeadAsset.add(rs2.getString("headTitle"));
				FirstHeadAssetId.add(rs2.getString("headid"));
			}
			
			for(int count=0;count<FirstHeadAsset.size();count++){
				ResultSet rs5=db.sta.executeQuery("select accfhead.headid,accfhead.headTitle from accfhead where accfhead.pheadId='"+FirstHeadAssetId.get(count)+"'");
				
				table.setValueAt("   *"+FirstHeadAsset.get(count), k, 0);
				k++;
				//nrow++;
				int j=k;
				while(rs5.next()){
			...
			table.setValueAt(df.format(sum1), k, 2);
	...
	private void addcmp() {
		...
		mainPanel.setPreferredSize(new Dimension(950,670));
	...
	private void northPanel_work() {
		TitledBorder titlemain=BorderFactory.createTitledBorder("Trail Balance");
