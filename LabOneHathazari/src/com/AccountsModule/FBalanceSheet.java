...
public class FBalanceSheet extends JPanel{
	...
	int k=0;
	public FBalanceSheet(SessionBeam sessionbeam) {
	...
	private void btnprintEvent(){
		try {
			...
			String input = "AccountReport/BalanceSheetFinal.jrxml";
	...
	private void FindBalanceSheet() {
		
		double ProfitLoss=FindProfitLosse();
		refreshTable();
		showData();
		...
		try {
			...
			table.setValueAt(ProfitLoss, k, 2);
			k++;
			...
			for(int count=0;count<FirstHeadAsset.size();count++){
				...
				if(find!=0){
					...
					String id="";
					ResultSet result=db.sta.executeQuery("select headid from accfhead where headTitle='"+FirstHeadAsset.get(count)+"'");
					while(result.next()){
						id=result.getString("headid");
					}
					findSubhead(id);
					findLedger();
					DebitledgerNewDetails(k-1,1);
					sum=sum+Double.parseDouble(table.getValueAt(k-1, 1).toString());
					findlist.clear();
				}
			...
			table.setValueAt(df.format(sum1+ProfitLoss), k, 2);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void CreditLedgerNewDetails(int row,int col){
		try {
			...
			for(int b=0;b<ledgerlist.size();b++){
				//System.out.println("ledgerlist "+ledgerlist.get(b).toString());
				...
				if(ledgerlist.get(b).toString().equals("561") || ledgerlist.get(b).toString().equals("565")){
	...
	private double FindProfitLosse(){
		double netIncome=0;
		try {
			...
			netIncome=MinusGrossPorift+netProfitSum;
			k++;
...
/*			table.setValueAt("      Total", k, 0);
			...
			if(netIncome>0){
				table.setValueAt(df.format(netIncome),k, 2);
			}
			else{
				table.setValueAt(df.format(netIncome*-1),k, 1);
			}
	...
	private void northPanel_work() {
		TitledBorder titlemain=BorderFactory.createTitledBorder("Balance Sheet");
	...
	private void southPanel_work() {
		southPanel.setPreferredSize(new Dimension(12,570));
