...
public class CashTransectionReportIncomeExpense extends JPanel{
	...
	int Tledger=0;
	public CashTransectionReportIncomeExpense(SessionBeam sessionbeam) {
	...
	public void incomestatement(){
		try {
			...
			for(int count=0;count<FirstHeadExepense.size();count++){
				...
				int j=i;
				while(rs5.next()){
					if(!rs5.getString("headTitle").toString().equals("Lab Discount") && !rs5.getString("headTitle").toString().equals("Hospital Discount")){
			...
			int k=0;
			table.setValueAt("   *Trunover", k, 2);
			k++;
			table.setValueAt("      Lab Sale", k, 2);
			
			double LabAmount=0;
			ResultSet rs1=db.sta.executeQuery("select ISNULL(sum(accftransection.Amount),0) as TotalAmount from  accftransection where UnitId='2' and DepId='4' and d_l_id=1 and entryTime between '"+starDate+"' and '"+endDate+"' ");
			while(rs1.next()){
				LabAmount=LabAmount+Double.parseDouble(rs1.getString("TotalAmount"));
			}
			table.setValueAt(LabAmount, k, 3);
			sum1=sum1+Double.parseDouble(table.getValueAt(k, 3).toString());
			k++;
			table.setValueAt("      Hospital Sale", k, 2);
			double HospitalSale=0;
			ResultSet rs2=db.sta.executeQuery("select ISNULL(sum(accftransection.Amount),0) as TotalAmount from  accftransection where UnitId='1' and (DepId='1' or DepId='2' or DepId='3' or DepId='5')  and d_l_id=1 and entryTime between '"+starDate+"' and '"+endDate+"' ");
			while(rs2.next()){
				HospitalSale=HospitalSale+Double.parseDouble(rs2.getString("TotalAmount"));
			}
			table.setValueAt(HospitalSale, k, 3);
			sum1=sum1+Double.parseDouble(table.getValueAt(k, 3).toString());
			k++;

			table.setValueAt("      Doctor's Sale", k, 2);
			double DoctorSale=0;
			ResultSet rs3=db.sta.executeQuery("select ISNULL(sum(accftransection.Amount),0) as TotalAmount from  accftransection where UnitId='1' and DepId='7' and d_l_id=1 and entryTime between '"+starDate+"' and '"+endDate+"' ");
			while(rs3.next()){
				DoctorSale=DoctorSale+Double.parseDouble(rs3.getString("TotalAmount"));
			}
			table.setValueAt(DoctorSale, k, 3);
			sum1=sum1+Double.parseDouble(table.getValueAt(k, 3).toString());
			k++;
			
			table.setValueAt("      Dialysis Sale", k, 2);
			double DialysisSale=0;
			ResultSet rs4=db.sta.executeQuery("select ISNULL(sum(accftransection.Amount),0) as TotalAmount from  accftransection where UnitId='1' and DepId='8' and d_l_id=1 and entryTime between '"+starDate+"' and '"+endDate+"' ");
			while(rs4.next()){
				DialysisSale=DoctorSale+Double.parseDouble(rs4.getString("TotalAmount"));
			}
			table.setValueAt(DialysisSale, k, 3);
			sum1=sum1+Double.parseDouble(table.getValueAt(k, 3).toString());
			k++;
	...
	public void incomestatementreport_work(){
		try {
			...
			String input = "AccountReport/DailyCashTransectionIncomeStatement.jrxml";
	...
	private void addcmp() {
		...
		mainPanel.setPreferredSize(new Dimension(950,660));
	...
	private void northPanel_work() {
		TitledBorder titlemain=BorderFactory.createTitledBorder("Cash Transection For Income Expense");
