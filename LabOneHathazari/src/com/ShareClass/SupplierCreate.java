...
public class SupplierCreate extends JPanel{
	...
	JButton btnPrint=new JButton("Print Supplier List",new ImageIcon("icon/print.png"));
	JButton btnReset=new JButton("Reset",new ImageIcon("icon/reresh.png"));
	String startDate="",ledgerName="",supplierId="";
	int ledgerId=0;
	BufferedImage image;
	public SupplierCreate(SessionBeam sessionbeam) {
	...
	private void btnAction(){
		table.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent arg0) {
				try {
					ResultSet rs=db.sta.executeQuery("select *from tbsupplierinfo where SupplierId='"+table.getValueAt(table.getSelectedRow(), 0)+"'");
					while(rs.next()){
		...
		checkAsc.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(checkAsc.isSelected()){
					...
					try {
						ResultSet rs=db.sta.executeQuery("select *from tbsupplierinfo order by tbsupplierinfo.supplierId,tbsupplierinfo.supplierName asc");
						while(rs.next()){
		...
		checkDsc.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(checkDsc.isSelected()){
					...
					try {
						ResultSet rs=db.sta.executeQuery("select *from tbsupplierinfo order by tbsupplierinfo.supplierId desc");
						while(rs.next()){
							model.addRow(new Object[]{rs.getString("SupplierId"),rs.getString("SupplierName"),rs.getString("mobile"),rs.getString("address")});
						}
					} catch (Exception e) {
...
/*	private void a(){
		try {
			for(int a=0;a<table.getRowCount();a++){
				if(table.getValueAt(a, 1).toString()!=""){
					...
					String cLeger="update tbsupplierinfo set acc_ledger='"+ledgerId+"' where supplierName='"+table.getValueAt(a, 1).toString()+"' ";
	...
	private void btnFineEvent() {
		try {
			...
			ResultSet rs=db.sta.executeQuery("select *from tbsupplierinfo where  tbsupplierinfo.supplierName='"+cmbSearchSupplierName.txtMrNo.getText().trim().toString()+"'");
			while(rs.next()){
				model.addRow(new Object[]{rs.getString("SupplierId"),rs.getString("SupplierName"),rs.getString("mobile"),rs.getString("address")});
				txtSupplierId.setText(rs.getString("SupplierId"));
				txtSupplierName.setText(rs.getString("SupplierName"));
	...
	public void loadSupplierName(){
		try {
			...
			ResultSet rs=db.sta.executeQuery("select  tbsupplierinfo.supplierName from tbsupplierinfo order by tbsupplierinfo.supplierName ");
			while(rs.next()){
	...
	public void Supplierreport_work(){
		try {
			...
			jq.setText("select *from tbsupplierinfo");
			jd.setQuery(jq);
	...
	private void ViewTableData(){
		try {
			...
			ResultSet rs=db.sta.executeQuery("select *from tbsupplierinfo ");
			while(rs.next()){
				model.addRow(new Object[]{rs.getString("SupplierId"),rs.getString("SupplierName"),rs.getString("mobile"),rs.getString("address")});
			}
			for(int a=0;a<20;a++){
			...
			if(!txtContactNumber.getText().toString().isEmpty()){
				if(!doplicateName()){
					try {
						...
						System.out.println(query);
						db.sta.executeUpdate(query);
						String cLeger="update tbsupplierinfo set acc_ledger='"+ledgerId+"' where  supplierName='"+txtSupplierName.getText().toString()+"' ";
						System.out.println(cLeger);
						db.sta.executeUpdate(cLeger);
				...
				if(doplicateName()){
					try {
						...
						if(confrim==JOptionPane.YES_OPTION){
							String sql="update tbsupplierinfo set SupplierId='"+txtSupplierId.getText().toString()+"',SupplierName='"+txtSupplierName.getText().toString()+"',mobile='"+txtContactNumber.getText().toString()+"',email='"+txtEmail.getText().toString()+"',address='"+txtAddress.getText().toString()+"',openingBalance='"+txtOpeningBalance.getText().trim().toString()+"',date=CURRENT_TIMESTAMP,entryTime=CURRENT_TIMESTAMP,createBy='"+sessionbeam.getUserId()+"' where SupplierId='"+txtSupplierId.getText().toString()+"'";
							System.out.println(sql);
							db.sta.executeUpdate(sql);
	...
	private void PrintBtnEvent(){
		try {
			
			
			String sql="";
			String report="IndoorPhamarcyAllReport/SupplierLIst.jrxml";
			sql="select supplierId,supplierName,mobile,address from tbSupplierInfo order by supplierName";
	...
	private void autoId(){
		try {
			String sql="select (ISNULL(max(SupplierId),0)+1)as SupplierId from tbsupplierinfo";
