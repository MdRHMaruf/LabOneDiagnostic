...
public class UserCreate extends JPanel
{
	...
	String department[]={"Setting","Receiption","Indoor Pharmacy","Outdoor Pharmacy","Store","Lab & Pathology","Billing","Opd Billing","Accounts"};
...
/*		public boolean isCellEditable(int row,int col){
	...
	String moduleName[]={"Setting","Receiption","Indoor Pharmacy","Outdoor Pharmacy","Store","Lab & Pathology","Billing","Opd Billing","Accounts"};
		...
		/*				if(rs1.getString("userType").toString().equals("Admin")){
					...
					for(int a=0;a<accessTable.getRowCount();a++){
						while(rs2.next()){
							...
							if(accessTable.getValueAt(8, 1).toString().equalsIgnoreCase(rs2.getString("moduleName"))){
								accessmodel.setValueAt(Boolean.parseBoolean(rs2.getString("status").toString().replace("1", new Boolean(true).toString())),8, 2);
							}
						}
					}
				}
