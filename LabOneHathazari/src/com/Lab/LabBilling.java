...
public class LabBilling extends JPanel{
	...
	JLabel lblPercentDiscount=new JLabel("D.Discount % ");
	JTextField txtPercentDiscount=new JTextField(3);
	...
	JLabel lblGeneralDiscount=new JLabel("D.Discount");
	...
	public void btnActionEvent(){
		...
		txtPaidInCash.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent e) {
				if(txtPaidInCash.getText().trim().isEmpty())
				{
					txtPaidInCash.setText("0");
				}
			}
		...
		txtPercentDiscount.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent arg0) {
				// TODO Auto-generated method stub
				if(txtPercentDiscount.getText().trim().isEmpty()) { txtPercentDiscount.setText("0"); txtGeneralDiscount.setText("0");}
		...
		txtCO.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				if(e.getKeyCode()==KeyEvent.VK_ENTER){
					...
					btnPost.requestFocusInWindow();
				}
			}
		});
		btnPost.addKeyListener(new KeyListener() {
		...
		cmbRefferBy.txtMrNo.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				try {
					...
					if(e.getKeyCode()==KeyEvent.VK_DOWN|| e.getKeyCode()==KeyEvent.VK_UP) {
						...
						String sql="select Degree from tbdoctorinfo where Name='"+cmbRefferBy.txtMrNo.getText().trim()+"'";
						System.out.println(sql);
						ResultSet rs=db.sta.executeQuery(sql);
						if(rs.next()) {
							txtDoctorDegree.setText(rs.getString("Degree"));
						}else {
		...
		/*		txtManualDiscount.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent arg0) {
				...
				if(!txtManualDiscount.getText().trim().toString().isEmpty()){
					...
					double tpayable=Double.parseDouble(txtTotalCharge.getText().toString())-(Double.parseDouble(txtGeneralDiscount.getText().toString())+Double.parseDouble(txtSpDiscount.getText().toString()));
					txtTotalPayable.setText(df.format(tpayable));
					if(Double.parseDouble(txtTotalPayable.getText().trim().toString())>Double.parseDouble(txtPaidInCash.getText().trim().toString()) && find == 0){
	...
	protected boolean checkAppliableDiscount() {
		double manualDiscount=Double.parseDouble(txtManualDiscount.getText().trim().toString());
		double payable=Double.parseDouble(txtTotalPayable.getText().trim().toString());
		double dueAmount=Double.parseDouble(txtDues.getText().trim().toString());
		if(payable<0) {
			JOptionPane.showMessageDialog(null,"Now Payable Amount is = "+payable+" for Manual discount = "+manualDiscount+"\nYour Manual discount will be = "+(manualDiscount+payable), "Sorry", JOptionPane.INFORMATION_MESSAGE);
			txtManualDiscount.setText(String.valueOf(manualDiscount+payable));
			txtSpDiscount.setText(String.valueOf(manualDiscount+payable));
			autoCalculation();
			return false;
		}
		if(dueAmount<0) {
			JOptionPane.showMessageDialog(null,"This patient already paid = "+(payable-dueAmount)+"\nSo...Manual discount = "+manualDiscount+" is not acceptable\nYour Manual discount will be = "+(manualDiscount+dueAmount), "Sorry", JOptionPane.INFORMATION_MESSAGE);
			txtManualDiscount.setText(String.valueOf(manualDiscount+dueAmount));
			txtSpDiscount.setText(String.valueOf(manualDiscount+dueAmount));
			autoCalculation();
			return false;
		}
		return true;
					...
					if(!txtAge.getText().trim().toString().isEmpty() ||  txtMonth.getText().trim().toString().isEmpty() || txtDay.getText().trim().toString().isEmpty()){
						if(txtDate.getValue()!=null){
							if(checkAppliableDiscount()) {
								...
								JOptionPane.showMessageDialog(null, "Please check your Discount..","Attention...",JOptionPane.WARNING_MESSAGE);
							}
	...
	protected void autoCalculation() {
		...
		if(Double.parseDouble(txtTotalPayable.getText().trim().toString())>Double.parseDouble(txtPaidInCash.getText().trim().toString()) ){
			...
			double tpaid= find==0?Double.parseDouble(txtPaidInCash.getText().trim().toString()):Double.parseDouble(txtAdvancepaid.getText().trim().toString());
			txtDues.setText(df.format(Double.parseDouble(txtTotalPayable.getText().trim().toString())-tpaid));
		}
		else{
			txtDues.setText("0.0");
			//txtRefund.setText(df.format(Double.parseDouble(txtPaidInCash.getText().trim().toString())-Double.parseDouble(txtTotalPayable.getText().trim().toString())));
			...
			if(confrim==JOptionPane.YES_OPTION){
				try {
					...
					if(sessionBeam.getUserType().equalsIgnoreCase("Admin")){
						...
						String sql="update tblabpatient set PatientName='"+txtPatientName.getText().trim().toString()+"',Mobile='"+txtPhone.getText().trim().toString()+"',Age='"+txtAge.getText().trim().toString()+"',Month='"+txtMonth.getText().trim().toString()+"',day='"+txtDay.getText().trim().toString()+"',Sex='"+cmbsex.getSelectedItem().toString()+"',address='"+txtAddress.getText().trim().toString()+"',Cabin='"+txtBC.getText().trim().toString()+"',RefferBy='"+refferId+"',RfPersionId='"+RfPersionId+"',ComissionAllow='"+ComissionAllow+"',CO='"+txtCO.getText().trim().toString()+"',SampleCollect='"+txtSampleCollection.getText().trim().toString()+"',ReportDelivery='"+new SimpleDateFormat("dd-MM-yyyy").format(txtDate.getValue())+" : "+txtTime.getText().toString()+"',ManualDiscount='"+txtManualDiscount.getText().trim().toString()+"',totalDiscount='"+Totaldiscount+"',TotalPayable='"+txtTotalPayable.getText().trim().toString()+"',entryTime=CURRENT_TIMESTAMP,ModifyBy='"+sessionBeam.getUserId()+"' where labId='"+txtMrNo.getText().trim().toString()+"' and FiscalYear='"+txtBillFiscalYear.getText().trim().toString()+"' and CMonth='"+txtBillMonth.getText().trim().toString()+"' ";
						System.out.println(sql);
						db.sta.executeUpdate(sql);
					}
					else{
					...
					String BfTransId="";
					ResultSet rs=db.sta.executeQuery("select TransId from TbLabPatient where labId='"+txtMrNo.getText().trim()+"' and FiscalYear = '"+txtBillFiscalYear.getText().trim()+"' and CMonth='"+txtBillMonth.getText().trim()+"' ");
					while(rs.next()){
					...
					actualDue = Double.parseDouble(txtDues.getText().trim().toString());
	...
	private void btnAddDoctorEvent(){
		try {
			if(!checkDoctorName()){
				...
				String sql="insert into tbdoctorinfo (autoId,DoctorCode,Name,Mobile,Degree,type) values ('"+DAutoId+"','"+DCCode+"','"+cmbRefferBy.txtMrNo.getText().trim().toString()+"','0','"+txtDoctorDegree.getText().trim().toString()+"','Referral')";
				db.sta.executeUpdate(sql);
				loadDoctorName();
			}
					...
					try {
						...
						if(refund!=0){
							...
							if(new  SimpleDateFormat("yyyy-MM-dd").format(txtDateTime.getDate()).equals(new  SimpleDateFormat("yyyy-MM-dd").format(new Date()))){
								refundType = "New Refund";
							}else {
								refundType = "Old Refund";
							}
							AutoPaymentHistoryId();
							String type=cmbPatientType.getSelectedIndex()==1?"Indoor":"Outdoor";
							String paymentquery="insert into TbLabPaymentHistory values('"+paymentId+"','"+Bill+"','"+RefundAmount+"','0','"+type+"','Refund',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'"+sessionBeam.getUserId()+"','"+FiscalYear+"','"+BillMonth+"','"+refundType+"')";
							System.out.println(paymentquery);
							db.sta.executeUpdate(paymentquery);

							String Udpaymentquery="insert into TbUdLabPaymentHistory values('"+paymentId+"','"+Bill+"','"+RefundAmount+"','0','"+type+"','Refund',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'"+sessionBeam.getUserId()+"','NEW','"+FiscalYear+"','"+BillMonth+"','"+refundType+"')";
							...
							String BfTransId="";
							ResultSet rs1=db.sta.executeQuery("select TransId from TbLabPatient where labId='"+Bill+"' and FiscalYear = '"+FiscalYear+"' and CMonth='"+BillMonth+"'  ");
							while(rs1.next()){
			...
			if(checkBillFound(BillId,FiscalYear,BillMonth)){
				try {
					...
					while(rs.next()){
						...
						actualDue=(Double.parseDouble(rs.getString("TotalPayable"))+Double.parseDouble(rs.getString("Refund")))-Double.parseDouble(rs.getString("Paid"));
						txtDues.setText(actualDue<0?"0":Double.toString(actualDue));
	...
	public void ConfirmWithStatus(int i){
		try {
			...
			if(find==0){
				...
				if(Double.parseDouble(txtPaidInCash.getText().trim().toString())<=actualDue){
					date_take();
					if(Double.parseDouble(txtPaidInCash.getText().trim().toString())>0.0){
						insertdata(txtMrNo.getText().trim().toString(),PatientFiscaleYear,"2",refferId,RfPersionId,CommissionAllow,i);
					}else
						JOptionPane.showMessageDialog(null,"Please enter cash amount... ","Sorry...",JOptionPane.INFORMATION_MESSAGE);
				}
				else{
					JOptionPane.showMessageDialog(null,"You can not recive more than due amount..\n Due Amount is = "+actualDue,"Sorry...",JOptionPane.WARNING_MESSAGE);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			...
			if(find==0){
				...
				if(cash<=actualAmt){
					...
					String BfTransId="";
					ResultSet rs=db.sta.executeQuery("select TransId from TbLabPatient where labId='"+MrNo+"' and FiscalYear = '"+FiscalYear+"' and CMonth='"+BillMonth+"' ");
					while(rs.next()){
						BfTransId=rs.getString("TransId");
					}
					...
					if(Double.parseDouble(txtPaidInCash.getText().trim().toString())>0){
						...
						String CashUpdate="update tblabpatient set CashId=CashId+',"+TransId+"' where   labId='"+MrNo+"' and FiscalYear = '"+FiscalYear+"' and CMonth='"+BillMonth+"'  ";
						db.sta.executeUpdate(CashUpdate);

						String UdCashUpdate="update tbUdlabpatient set CashId=CashId+',"+TransId+"' where  labId='"+MrNo+"' and FiscalYear = '"+FiscalYear+"' and CMonth='"+BillMonth+"'  ";
						db.sta.executeUpdate(UdCashUpdate);
					}
	...
	public void btnClearEvent() {
		...
		try {
			...
			Date userTime=formatTime.parse("13:30:00");

			String current= formatTime.format(new Date());
			Date currentTime=formatTime.parse(current);

			if(userTime.before(currentTime)) {
				Calendar c = Calendar.getInstance();
				c.setTime(new Date());
				c.add(Calendar.DATE, 1); 
				txtDate.setValue(c.getTime());
					...
					if(!checkDoplicateTest(TestName)){
						...
						if(find==0){
							...
							if(checkUserTypeIsAdmin()){
								...
								for(int a=0;a<count;a++){
									if(!checkInvoiceDoublePerticular("select tblabtesthistory.TestName from tblabtesthistory where tblabtesthistory.FiscalYear='"+FiscalYear+"' and tblabtesthistory.labId='"+Bill+"' and tblabtesthistory.CMonth='"+BillMonth+"' and tblabtesthistory.TestName='"+NameOfPericular.get(a).toString()+"' ")){
										autoId();

										String query ="insert into tblabtesthistory (labId,regNo,SN,testCode,testName,qty,rate,discount,discountAllow,ResultStatus,RefundStatus,type,counter,date,entryTime,createBy,FiscalYear,CMonth) values ('"+Bill+"' ,'"+RegNo+"','"+SN.get(a).toString()+"','"+TId.get(a).toString()+"','"+NameOfPericular.get(a).toString()+"','"+NameOfQty.get(a).toString()+"','"+df.format(Double.parseDouble(NameOfRate.get(a).toString()))+"','0','0','NOT DONE','0','2','"+counter+"',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'"+sessionBeam.getUserId()+"','"+FiscalYear+"','"+BillMonth+"')";
										System.out.println(query);
										db.sta.executeUpdate(query);

										String Udquery ="insert into tbUdlabtesthistory (labId,regNo,SN,testCode,testName,qty,rate,discount,discountAllow,ResultStatus,RefundStatus,type,counter,date,entryTime,createBy,flag,FiscalYear,CMonth) values ('"+Bill+"' ,'"+RegNo+"','"+SN.get(a).toString()+"','"+TId.get(a).toString()+"','"+NameOfPericular.get(a).toString()+"','"+NameOfQty.get(a).toString()+"','"+df.format(Double.parseDouble(NameOfRate.get(a).toString()))+"','0','0','NOT DONE','0','2','"+counter+"',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'"+sessionBeam.getUserId()+"','NEW','"+FiscalYear+"','"+BillMonth+"')";
										System.out.println(Udquery);
										db.sta.executeUpdate(Udquery);
									}
								}
								...
								showData("select *from tblabtesthistory where tblabtesthistory.FiscalYear='"+FiscalYear+"' and tblabtesthistory.labId='"+Bill+"' and tblabtesthistory.Cmonth='"+BillMonth+"' order by type asc");
								...
								String UdUpdatesql="insert into tbUdlabpatient values('"+txtMrNo.getText().trim().toString()+"',"
										+ "'"+patienttype+"',"
										+ "'"+txtMrNo.getText().trim().toString()+"',"
										+ "'"+RegNo+"',"
										+ "'"+txtPatientName.getText().trim().toString()+"',"
										...
										+ "'"+txtPaidInCash.getText().trim().toString()+"',"
										+ "CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,"
										+ "'"+sessionBeam.getUserId()+"','"+sessionBeam.getUserId()+"','UPDATE','0','','"+getFiscelYear()+"','"+getCurrentMonth()+"',NULL,'0','0','0','0')";
								System.out.println(UdUpdatesql);
								...
								String BfTransId="";
								ResultSet rs=db.sta.executeQuery("select TransId from TbLabPatient where labId='"+Bill+"' and FiscalYear = '"+FiscalYear+"' and CMonth='"+BillMonth+"' ");
								while(rs.next()){
									BfTransId=rs.getString("TransId");
								}
								...
								JOptionPane.showMessageDialog(null, "New Test Entry Successfull!!");
							}
							else{
								JOptionPane.showMessageDialog(null, "You have no permission to add new test in this bill");
							}
	...
	private double getCmdDiscount(String TestName){
		...
		try {
			int temp=0;
			String sql="select ISNULL(DoctorComission,0) as DoctorComission from TbDoctorComissionSet where TestGroupId=(select TestHeadId from tbtestname where TestName='"+TestName+"') and DoctorId=(select DoctorCode from tbdoctorinfo where Name='"+cmbRFPersion.txtMrNo.getText().trim().toString()+"')";
	...
	private boolean checkUserTypeIsAdmin(){
		try {
			ResultSet rs=db.sta.executeQuery("select username from tblogin where user_id='"+sessionBeam.getUserId()+"' and userType='Admin'");
			while(rs.next()){
				return true;
			}
		} catch (Exception e) {
						...
						if(checkIsTestName(TestName)){
							...
							if(find==0){
								...
								if(checkUserTypeIsAdmin()){
									...
									for(int a=0;a<Dcount;a++){
										if(checkInvoiceDoublePerticular("select tblabtesthistory.TestName from tblabtesthistory where tblabtesthistory.FiscalYear='"+FiscalYear+"' and tblabtesthistory.labId='"+Bill+"' and tblabtesthistory.Cmonth='"+BillMonth+"' and tblabtesthistory.TestName='"+DNameOfPericular.get(a).toString()+"'")){
											String deleteQ="delete from tblabtesthistory where tblabtesthistory.FiscalYear='"+FiscalYear+"' and tblabtesthistory.labId='"+Bill+"' and tblabtesthistory.Cmonth='"+BillMonth+"' and tblabtesthistory.TestName='"+DNameOfPericular.get(a).toString()+"' and tblabtesthistory.type=2 ";
											//System.out.println(deleteQ);
											db.sta.executeUpdate(deleteQ);
										}
									}
									
									String deleteQ="delete from tblabtesthistory where tblabtesthistory.FiscalYear='"+FiscalYear+"' and tblabtesthistory.labId='"+Bill+"' and tblabtesthistory.CMonth='"+BillMonth+"'  and tblabtesthistory.TestName='"+TestName+"' and tblabtesthistory.type=1 ";
									//System.out.println(deleteQ);
									db.sta.executeUpdate(deleteQ);
									
									String deleteQT="delete from tblabtesthistory where tblabtesthistory.FiscalYear='"+FiscalYear+"' and tblabtesthistory.labId='"+Bill+"'  and tblabtesthistory.CMonth='"+BillMonth+"'  and tblabtesthistory.type=2";
									//System.out.println(deleteQT);
									db.sta.executeUpdate(deleteQT);
									
	/*								String deleteQ="delete from tblabtesthistory where tblabtesthistory.FiscalYear='"+FiscalYear+"' and tblabtesthistory.labId='"+Bill+"' and tblabtesthistory.TestName='"+TestName+"' and tblabtesthistory.type=1 ";
									System.out.println(deleteQ);
									db.sta.executeUpdate(deleteQ);
									String deleteQT="delete from tblabtesthistory where tblabtesthistory.FiscalYear='"+FiscalYear+"' and tblabtesthistory.labId='"+Bill+"' and tblabtesthistory.type=2";
									System.out.println(deleteQT);
									db.sta.executeUpdate(deleteQT);*/

									//String DPaymentQuery="delete from TbLabPaymentHistory where tblabtesthistory.FiscalYear='"+FiscalYear+"' and BillNo='"+Bill+"' and PaymentStatus='Paid'";
									...
									for(int a=0;a<table.getRowCount();a++){
										if(table.getValueAt(a, 2).toString()!=""){
											...
											for(int b=0;b<count;b++){
												String sql="select tblabtesthistory.TestName from tblabtesthistory where tblabtesthistory.TestName='"+NameOfPericular.get(b).toString()+"' and tblabtesthistory.FiscalYear='"+FiscalYear+"' and tblabtesthistory.labId='"+Bill+"' and tblabtesthistory.CMonth='"+BillMonth+"'";
										
												if(!checkInvoiceDoublePerticular(sql)){
													autoId();
													System.out.println("insert ");
													String query ="insert into tblabtesthistory (labId,regNo,SN,testCode,testName,qty,rate,discount,ResultStatus,RefundStatus,type,counter,date,entryTime,createBy,FiscalYear,CMonth) values ('"+Bill+"','"+RegNo+"','"+SN.get(b).toString()+"','"+TId.get(b).toString()+"','"+NameOfPericular.get(b).toString()+"','"+NameOfQty.get(b).toString()+"','"+df.format(Double.parseDouble(NameOfRate.get(b).toString()))+"','0','NOT DONE','0','2','"+counter+"',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'"+sessionBeam.getUserId()+"','"+FiscalYear+"','"+BillMonth+"')";
													System.out.println(query);
													db.sta.executeUpdate(query);

													String Udquery ="insert into tbUdlabtesthistory (labId,regNo,SN,testCode,testName,qty,rate,discount,ResultStatus,RefundStatus,type,counter,date,entryTime,createBy,FiscalYear,CMonth) values ('"+Bill+"','"+RegNo+"','"+SN.get(b).toString()+"','"+TId.get(b).toString()+"','"+NameOfPericular.get(b).toString()+"','"+NameOfQty.get(b).toString()+"','"+df.format(Double.parseDouble(NameOfRate.get(b).toString()))+"','0','NOT DONE','0','2','"+counter+"',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'"+sessionBeam.getUserId()+"','"+FiscalYear+"','"+BillMonth+"')";
													System.out.println(Udquery);
													db.sta.executeUpdate(Udquery);
												}
											}									
									...
									System.out.println(sql);
									db.sta.executeUpdate(sql);
									String patienttype=cmbPatientType.getSelectedItem().toString();
									...
									String Udsql="insert into tbUdlabpatient values('"+txtMrNo.getText().trim().toString()+"',"
											...
											+ "'"+txtPaidInCash.getText().trim().toString()+"',"
											+ "CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,"
											+ "'"+sessionBeam.getUserId()+"','"+sessionBeam.getUserId()+"','UPDATE','0','"+PatientFiscalYear+"','"+getFiscelYear()+"','"+getCurrentMonth()+"',NULL,'0','0','0','0')";
									System.out.println(Udsql);
									db.sta.executeUpdate(Udsql);
									...
									JOptionPane.showMessageDialog(null, "You Have no permission to delete any test");
								}
