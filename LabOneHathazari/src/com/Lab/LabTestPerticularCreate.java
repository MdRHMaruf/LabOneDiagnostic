...
/*								else{
	...
	private boolean checkEditAuthentication(){
		try {
			ResultSet rs=db.sta.executeQuery("select * from tbLogin where user_id='"+sessionBeam.getUserId()+"' and change='1'");
			while(rs.next()){
				return true;
			}
		} catch (Exception e) {
	...
	private boolean checkDeletetAuthentication(){
		try {
			ResultSet rs=db.sta.executeQuery("select * from tbLogin where user_id='"+sessionBeam.getUserId()+"' and clear='1'");
			while(rs.next()){
				return true;
			}
		} catch (Exception e) {
			...
			public void actionPerformed(ActionEvent e)
			{
				try {
					...
					if(confrim==JOptionPane.YES_OPTION){
						...
						JOptionPane.showMessageDialog(null, "You Have No Permission To Edit Test Perticular");
					}
				} catch (Exception e2) {
					e2.printStackTrace();
