...
public class BioChemestry extends JPanel{
	...
	String Col[]={"T.P.ID","Test Perticulars","Test Result","GFR Calculate","Flag","Normal Ranges","Uom","Sorting","Test Name"};
	Object Row[][]={};
...
/*		@Override
	...
	public BioChemestry(SessionBeam sessionBeam){
	...
	private void BtnActionEvent(){
		btnDiabetesTest.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub
				if(btnDiabetesTest.isSelected()){
					//txtNote.setText("*In the absence of unequivocal hypergiycaemia result should bc confirmed by repeat testing.#* IFG:Impaired fastiing glucose,IGT: Impaired glucose tolerence.#GDM:Diagnosed when any of the following plasma glucose are met or exceeded on 75-g-OGTT at 24-28 weeks of gestation");
				}
...
/*		model.addTableModelListener(new TableModelListener() {
				...
				if(e.getType()==TableModelEvent.UPDATE){
					...
					if(Col==2){
						if(!table.getValueAt(Row, Col).toString().isEmpty()){
							try {
								if(checkSugarCalculation(table.getValueAt(Row, 1).toString())){
									double OriginalValue=Double.parseDouble(table.getValueAt(Row, 2).toString());
									double dividedValue=OriginalValue/18;
									table.setValueAt("("+df.format(dividedValue)+")", Row, 3);
								}
	...
	private boolean checkSugarCalculation(String TestName){
		try {
/*			ResultSet rs=db.sta.executeQuery("select * from tbsubtestname where SubTestName='"+TestName+"' s");
			while(rs.next()){
				return true;
			}*/
		} catch (Exception e) {
	...
	public void btnPrintEvent(String BillId,String Reg,String Name,String Age,String Month,String Day,String Sex,String Cabin,String Consultant,Date OrDate,String MainTestList,String FiscalYear,String BillMonth){
		try {
			...
			for(int r = 0; r <table.getRowCount(); r++){
				ara[r]=Integer.parseInt(table.getValueAt(r, 7).toString());
			}
			...
			for (int r = 0; r <table.getRowCount(); r++)
			{
					...
					for(int a=0;a<table.getRowCount();a++){
						if(ara[r]==Integer.parseInt(table.getValueAt(a, 7).toString().trim())){
							ColCount=0;
							if(!table.getValueAt(r, 2).toString().trim().equals("")){
								...
								map.put("CalculateValue",table.getValueAt(a, 3).toString().trim());
								map.put("Flag",table.getValueAt(a, 4).toString().trim());
								
								
								if(!table.getValueAt(a, 5).toString().trim().equals("")){
									normalRange=table.getValueAt(a, 5).toString().trim();
								}
								
								map.put("NormalRange",table.getValueAt(a, 5).toString().trim());
								map.put("Unit",table.getValueAt(a, 6).toString().trim());
								...
								map.put("MainTestName",table.getValueAt(a, 8).toString().trim());
								map.put("TestList",MainTestList);
								map.put("Note",cmbComment.txtMrNo.getText().trim().toString());
								list.add(map);
							}
						}
					}
			...
			if(normalRange!="1"){
				...
				String input = "NewFormetReport/BioChemistry2ColWithGroup.jrxml";
				JasperReport com=JasperCompileManager.compileReport(input);
				jp = JasperFillManager.fillReport(com, map, new JRBeanCollectionDataSource(list));
				JasperViewer.viewReport(jp, false);
				JasperPrintManager.printReport(jp, true);
	...
	public void setPrintData(String labPid,String testCode,String TestName,String labId,Date date,String FiscalYear,String BillMonth){
		try {
			...
			if(temp==0){
				...
				for(int a=0;a<i;a++){
					ResultSet rs2=db.sta.executeQuery("select value,CalculateValue,Flag,Sorting,(select Name from tbdoctorinfo where autoId=tblabreportvalue.LabInchargeId) as LabInchargeName,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.LabInchargeId) as LabInchargeDegree,(select Name from tbdoctorinfo where autoId=tblabreportvalue.CheckedById) as CheckedByName,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.CheckedById) as CheckedByDegree,(select Name from tbdoctorinfo where autoId=tblabreportvalue.Doctor1Id) as Doctor1Name,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.Doctor1Id) as Doctor1Degree,(select Name from tbdoctorinfo where autoId=tblabreportvalue.Doctor2Id) as Doctor2Name,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.Doctor2Id) as Doctor2Degree,(select username from tblogin where user_id=tblabreportvalue.createBy) as username from tblabreportvalue where FiscalYear='"+FiscalYear+"' and CMonth='"+BillMonth+"' and labPid=2 and labBillId='"+labId+"' and rId='"+testCode+"'");
					while(rs2.next()){
						username=rs2.getString("username");
						model.addRow(new Object[]{testCode,TestList.get(a),rs2.getString("value"),rs2.getString("CalculateValue"),rs2.getString("Flag"),NormalRangeList.get(a),UnitList.get(a),rs2.getString("Sorting"),TestName});
...
/*			for(int a=table.getRowCount()-1;a>=0;a--){
			...
			if(temp==1){
				...
				for(int a=count;a<(i+count);a++){
					ResultSet rs2=db.sta.executeQuery("select value,CalculateValue,Flag,Sorting,(select Name from tbdoctorinfo where autoId=tblabreportvalue.LabInchargeId) as LabInchargeName,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.LabInchargeId) as LabInchargeDegree,(select Name from tbdoctorinfo where autoId=tblabreportvalue.CheckedById) as CheckedByName,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.CheckedById) as CheckedByDegree,(select Name from tbdoctorinfo where autoId=tblabreportvalue.Doctor1Id) as Doctor1Name,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.Doctor1Id) as Doctor1Degree,(select Name from tbdoctorinfo where autoId=tblabreportvalue.Doctor2Id) as Doctor2Name,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.Doctor2Id) as Doctor2Degree,(select username from tblogin where user_id=tblabreportvalue.createBy) as username from tblabreportvalue where FiscalYear='"+FiscalYear+"' and CMonth='"+BillMonth+"' and labPid=2 and labBillId='"+labId+"' and rId='"+table.getValueAt(a, 0).toString()+"'");
					while(rs2.next()){
						
						table.setValueAt(rs2.getString("value"), a, 2);
						table.setValueAt(rs2.getString("CalculateValue"), a, 3);
						table.setValueAt(rs2.getString("Flag"), a, 4);
						table.setValueAt(rs2.getString("Sorting"), a, 7);
						username=rs2.getString("username");
						cmbLabIncharge.txtMrNo.setText((rs2.getString("LabInchargeName")==null?"":rs2.getString("LabInchargeName"))+(rs2.getString("LabInchargeDegree")==null?"":rs2.getString("LabInchargeDegree")));
						cmbCheckedBy.txtMrNo.setText((rs2.getString("CheckedByName")==null?"":rs2.getString("CheckedByName"))+(rs2.getString("CheckedByDegree")==null?"":rs2.getString("CheckedByDegree")));
						cmbDoctorName1.txtMrNo.setText((rs2.getString("Doctor1Name")==null?"":rs2.getString("Doctor1Name"))+(rs2.getString("Doctor1Degree")==null?"":rs2.getString("Doctor1Degree")));
		...
		try {				
				...
				for(int a=0;a<table.getRowCount();a++){
					if(table.getValueAt(a, 0).toString()!="" && table.getValueAt(a, 6).toString()!=""){
						...
						if(checkSugarCalculation(table.getValueAt(a, 1).toString())){
							double OriginalValue=Double.parseDouble(table.getValueAt(a, 2).toString());
							double dividedValue=OriginalValue/18;
							table.setValueAt("("+df.format(dividedValue)+")", a, 3);
						}
						
							if(!checkLabReport(table.getValueAt(a, 0).toString(),labId,2,table.getValueAt(a, 0).toString(),FiscalYear,BillMonth)){
								AutoId();
								insertdata(a,autoId,table.getValueAt(a, 0).toString(),table.getValueAt(a, 2).toString(),table.getValueAt(a, 3).toString(),table.getValueAt(a, 4).toString(),a,table.getValueAt(a, 0).toString(),table.getValueAt(a, 7).toString(),LabInchargeId,CheckedById,Doctor1Id,Doctor2Id,FiscalYear,BillMonth);
							}
							else{
								updateData(a,autoId,table.getValueAt(a, 0).toString(),table.getValueAt(a, 2).toString(),table.getValueAt(a, 3).toString(),table.getValueAt(a, 4).toString(),a,table.getValueAt(a, 0).toString(),table.getValueAt(a, 7).toString(),LabInchargeId,CheckedById,Doctor1Id,Doctor2Id,FiscalYear,BillMonth);
							}
			...
			//JOptionPane.showMessageDialog(null, "Bio Chemestry Report Sucessfully Complete");
		} catch (Exception e) {
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "Error!!,"+e);
		}
	}
	public void insertdata(int r,String autoID,String rId,String value,String CalculateValue,String Flag,int row,String TestCode,String Sorting,String LabInchargeId,String CheckedById,String Doctor1Id,String Doctor2Id,String FiscalYear,String BillMonth){
		try {
			noteAutoId();
			String sql="insert into tblabreportvalue (autoId,rId,value,CalculateValue,Flag,labPId,labBillId,testCode,Sorting,LabInchargeId,CheckedById,Doctor1Id,Doctor2Id,date,entryTime,createBy,FiscalYear,CMonth) values ('"+autoID+"',"
					+ "'"+rId+"',"
					+ "'"+value+"',"
					+ "'"+CalculateValue+"',"
					+ "'"+Flag+"','2',"
	...
	public void updateData(int r,String autoID,String rId,String value,String CalculateValue,String Flag,int row,String TestCode,String Sorting,String LabInchargeId,String CheckedById,String Doctor1Id,String Doctor2Id,String FiscalYear,String BillMonth){
		try {
			String sql="update tblabreportvalue set value='"+value+"',CalculateValue='"+CalculateValue+"',Flag='"+Flag+"',Sorting='"+Sorting+"',LabInchargeId='"+LabInchargeId+"',CheckedById='"+CheckedById+"',Doctor1Id='"+Doctor1Id+"',Doctor2Id='"+Doctor2Id+"',createBy='"+sessionBeam.getUserId()+"' where  FiscalYear='"+FiscalYear+"' and CMonth='"+BillMonth+"' and rId='"+rId+"' and labBillId='"+labbillId+"' and testCode='"+TestCode+"' ";
			System.out.println(sql);
			db.sta.executeUpdate(sql);
		} catch (Exception e) {
			e.printStackTrace();
...
/*	private void NorthPanel_work() {
	...
	private void CenterPanel_work() {
		...
		CenterPanel.add(btnDiabetesTest);
		
		gp.add(btnSingleTest);
		gp.add(btnGroupTest);
		gp.add(btnDiabetesTest);
