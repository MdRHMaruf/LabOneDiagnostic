...
public class TestWiseNoteCreate extends JPanel{
	...
	private String getTestHeadId(){
		String TestId="";
		try {
			ResultSet rs=db.sta.executeQuery("select TestHeadId from TbTestName where TestName='"+cmbTestName.txtMrNo.getText().trim().toString()+"'");
			while(rs.next()){
				TestId=rs.getString("TestHeadId");
			}
		} catch (Exception e) {
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "Error"+e);
